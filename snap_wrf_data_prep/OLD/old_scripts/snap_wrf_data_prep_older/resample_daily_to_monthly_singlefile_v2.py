...
if __name__ == '__main__':
	...
	variables = [ os.path.basename(i).upper() for i in glob.glob( os.path.join(base_path, 'daily', '*' ) ) if os.path.isdir( i ) and 'slurm' not in i ]
	...
	for group, wildcard in wildcards:
		...
		for variable in variables:
			...
			files = sorted( glob.glob( os.path.join( base_path, 'daily', variable.lower(), wildcard ) ) )
			...
			# dump to disk
			out_fn = os.path.join( os.path.join( base_path, 'monthly', variable.lower(), '_'.join([variable.lower(), 'wrf', group, 'monthly','-'.join([str(begin), str(end)]), metric]) + '.nc'  ) )
