...
if __name__ == '__main__':
    ...
    out_df_futures = pd.read_csv('/workspace/Shared/Tech_Projects/DeltaDownscaling/project_data/downscaled_hurs_temporary/TESTING_NEW_OLD_hurs_hur/futures_yukonwatershed_areamean_allmodels.csv', index_col=0, parse_dates=True)
    ...
    # historicals plotting -- HACK should be built into the function above but I really dont feel like making this pretty.
    import matplotlib
    matplotlib.use('agg')
    from matplotlib import pyplot as plt

    out_df_futures = pd.read_csv('/workspace/Shared/Tech_Projects/DeltaDownscaling/project_data/downscaled_hurs_temporary/TESTING_NEW_OLD_hurs_hur/historicals_yukonwatershed_areamean_allmodels.csv', index_col=0, parse_dates=True)
    futures_df = out_df_futures.loc[slice('1900','1999')]
    months = [ i.month for i in futures_df.index ]
    years = [ str(i.year)[:-1]+'0s' for i in futures_df.index ]
    ...
    for model, scenario in itertools.product(models, scenarios):
        ...
        sub_df.plot(kind='line', title='Comparison Of New and Old Relative Humidity Versions\n{} - {}\nMonthly Decadal Averages 1900s-1990s'.format(model.upper(), scenario.upper()))
        plt.tight_layout()
        plt.savefig('compare_relative_humidity_decadals_{}_{}.png'.format(model,scenario))
        plt.close()
