...
if __name__ == '__main__':
    ...
    # fill an interp value for T1
    if year > df.year.min():
        sub_df.iloc[0, np.where(df.columns == 'interp')[0]] = True
        ii, = df[ df.fn == sub_df.iloc[0].fn ].index
        interp_list1 = df.iloc[[ii-1, ii, ii+1]].fn.tolist()
        # sub_df.iloc[0, np.where(df.columns == 'interp_files')[0]] = interp_list1
        sub_df = sub_df.set_value( sub_df.index[0], 'interp_files', interp_list1 )
