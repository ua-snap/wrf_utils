...
def run( fn ):
    ...
    out_fn = fn.replace( '/daily','/daily_fix' )
    dirname, basename = os.path.split( out_fn )
    basename, ext = os.path.splitext( basename )

    # [NOTE]: naming below is hardwired to a naming that is identical (element-wise) 
    #    to this: 'ACSNOW_wrf_daily_gfdl_rcp85_2018_sum' (.nc removed above)
    variable, group, timestep, model, scenario, year, metric = basename.split( '_' )
    ...
    if len(flipped['data'].shape) == 3:
        ...
        new_ds = xr.Dataset( {variable:(['time',levelname,'yc', 'xc'], flipped['data'])},
                    coords={'xc': ('xc', np.flipud(x)[0,]),
                            'yc': ('yc', np.flipud(y)[:,0]),
                            'lon':(['yc','xc'], np.flipud(ds.lon.data) ),
                            'lat':(['yc','xc'], np.flipud(ds.lat.data) ),
    ...
    var_attrs.update( long_name=var_attrs_lookup[ variable ][ 'long_name' ],
                      ...
                      coordinates_="xc yc",
                      temporal_resampling='Dailies generated from hourlies using {}'.format( metric_lookup[metric] )
                    )
